...
# The pitch distribution is modeled as the sum of K gaussian
...
class LSTM_gaussian_mixture(Model_lop):
    ...
    def __init__(self,
        ...
        if weights_initialization is None:
            ...
            self.b_std = shared_zeros((self.K_gaussian * self.n_o), bias=0.5, name='b_std')
    ...
    def cost(self):
        ...
        gaussian_mix = (weights_prediction * g_ll.dimshuffle((1,2,0))).sum(axis=2)

        # Take log then sum : (B,T) -> scalar
        cost = - (T.log(gaussian_mix)).sum() / (self.batch_size * self.temporal_order)
        ###########################
    ...
    def get_train_function(self, piano, orchestra, optimizer, name):
        ...
        super(LSTM_gaussian_mixture, self).get_train_function()
    ...
    def get_validation_error(self, piano, orchestra, name):
        ...
        super(LSTM_gaussian_mixture, self).get_validation_error()
    ...
    def get_generate_function(self, piano, orchestra, generation_length, seed_size, batch_generation_size, name="generate_sequence"):
        ...
        super(LSTM_gaussian_mixture, self).get_generate_function()
        ...
        # Mask std
        std_mixture_reshape = T.reshape(std_mixture, (batch_generation_size, generation_length, self.K_gaussian, self.n_o))
        selected_std = std_mixture_reshape * mask_weights_dimshuffle
