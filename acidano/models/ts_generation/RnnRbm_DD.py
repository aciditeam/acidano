...
class RnnRbm(object):
    ...
    def gibbs_step(self, v):
        ...
        mean_h = T.nnet.sigmoid(T.dot(v, self.W) + self.bh_dynamic)
        h = self.rng.binomial(size=mean_h.shape, n=1, p=mean_h,
                              dtype=theano.config.floatX)
        mean_v = T.nnet.sigmoid(T.dot(h, self.W.T) + self.bv_dynamic)
    ...
    def cost_updates(self, optimizer):
        ...
        # pydotprint(self.v, '../Debug/v.html')
        # pydotprint(self.bv_dynamic, '../Debug/bv_dyn.html')
        # pydotprint(self.bh_dynamic, '../Debug/bh_dyn.html')
        # pydotprint(v_sample, '../Debug/v_sample.html')
