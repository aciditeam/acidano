...
def convert_mp3_to_wav(filename, new_name, sample_frequency):
    ext = filename[-4:]
    if(ext != '.mp3'):
        return
...
def convert_folder_to_wav(directory, sample_rate=44100):
    for file in os.listdir(directory):
        ...
        if file.endswith('.mp3'):
            convert_mp3_to_wav(filename=fullfilename, new_name=None, sample_frequency=sample_rate)
    return directory + 'wave/'


def read_wav_as_np(filename):
    data = wav.read(filename)
    np_arr = data[1].astype('float32') / 32768.0  # Normalize 16-bit input to [-1, 1] range
